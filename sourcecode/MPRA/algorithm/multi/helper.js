...
function groupAssignmentByProject(assignment) {
  ...
  function getTotalKPIByProject(assignment) {
    const grouped = groupAssignmentByProject(assignment);
    const result = {};
    for (const [projectKey, items] of grouped.entries()) {
      const kpi = { A: 0, B: 0, C: 0 };
      for (const a of items) {
        for (const k in a.kpi) {
          kpi[k] = (kpi[k] || 0) + a.kpi[k];
        }
      }
      result[projectKey] = kpi;
    }
    return result;
  }
  
  function getKPIOfEmployeesByProject(assignment) {
    const result = {}; // {projectKey: { empId: {A:..., B:...} }}
    for (const a of assignment) {
      const pk = a.task.projectKey || 'unknown';
      if (!result[pk]) result[pk] = {};
      if (a.assignee && a.assignee.id) {
        if (!result[pk][a.assignee.id]) result[pk][a.assignee.id] = { A: 0, B: 0, C: 0 };
        for (const k in a.kpi) {
          result[pk][a.assignee.id][k] += a.kpi[k];
        }
      }
    }
    return result;
